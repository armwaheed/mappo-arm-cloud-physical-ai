...
| run holds in corridor / stall, no motion / 运行卡在走廊判定、不动 | Not enough clear width BESIDE the box for the detour's tangent corner. / 箱子**旁边**没有足够净宽，绕行拐角放不下。 | ⚠️ **Do NOT move the box off the line — §5 puts it ON the line on purpose, and off it this demo proves nothing.** Widen the lane instead: at least one side needs ~0.9 m of clear width from the line, computed from **measured** radii (robot 0.40 m + box 0.28–0.33 m + 0.14 m clearance). / ⚠️ **不要把箱子挪到线外—— 第 5 节是特意把它放在连线上的，挪开就什么都没演示。** 要做的是加宽通道：至少一侧距连线有约 0.9 m 净宽，按**实测**半径计算（机器人 0.40 m + 箱子 0.28–0.33 m + 0.14 m 余量）。 |
