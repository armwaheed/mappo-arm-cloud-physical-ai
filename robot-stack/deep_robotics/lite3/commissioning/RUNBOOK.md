...
**Four** of the tools in this directory walk the robot: `gait_floor_probe.py`,
`actuator_gain_probe.py`, `axis_primitive_probe.py` and `arc_probe.py` — and on this
robot's only walked transport it is the last two that matter, not the first two.
`arc_probe.py` is the one that drives forward and yaw TOGETHER, so it is the only one here
that traces a curve rather than a line: it needs clear space in every direction, not a
lane. All four refuse to move without **both** `--live` and `--operator-ready`. Neither
flag is a formality.

**中文** —— `robot-stack/SAFETY.md` 管辖一切会让腿动起来的操作，不可跳过。
本目录下有**四个**工具会让机器人行走：`gait_floor_probe.py`、`actuator_gain_probe.py`、
`axis_primitive_probe.py` 和 `arc_probe.py` —— 在本机唯一真正走起来过的那条通道上，
起作用的是后两个，而不是前两个。`arc_probe.py` 会**同时**驱动前进轴和偏航轴，
因此它是这里唯一走出弧线而非直线的工具：它需要四周都有净空，而不是一条通道。
四者都必须**同时**给出 `--live` 和 `--operator-ready` 才会动。
这两个参数都不是走形式。
