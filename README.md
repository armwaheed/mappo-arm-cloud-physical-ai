...
| ✅ Lite3 live MAPPO navigation, first walks | [#13](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/13): four `--live` runs on release v8. The first refused pre-flight on a combinator `battery_level()` gap — fixed and regression-tested in this PR. Then: a 0.05→0.55 m/s ramp with the goal range closing 2.29→1.75 m (0.54 m walked) before a correct 58 s corridor hold on the box (y=−0.10 m; it must sit ≥0.9 m off the robot→chair line), and on run four **arrived**, 0.99 m from the chair. Battery gate enforced (50→47→35→28%), stall abort and safe park both exercised. |
