...
| ✅ A deployed tree names its own commit | `deploy/push-to-robot.sh` stamps git's root tree id, recomputed from the bytes on disk; `mappo_drive.py` refuses a tree that stops matching it — demonstrated on the Go2 under Python 3.8.10, with `git` never invoked. Six of the ten `~/mappo-*` trees turned out to be exact checkouts; `~/mappo-main`, the one the wrappers source, spans 21 commits ([#128](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/128)) |
| ✅ Maps a static obstacle, goes around it, detected goal | live; walked 1.89 m, stopped for lane width |
| ✅ Offline regression suite | Every count lives in [`.github/test-inventory.tsv`](.github/test-inventory.tsv), generated by `bash .github/measure-suites.sh --write` and **re-measured by CI on every pull request**, which fails if a number there disagrees in either direction. ⚠️ **No total is restated here on purpose.** This row has been wrong four times: it claimed 526, then 772, then 1,027 against an enforced 1,038, then 1,046 against an enforced 1,337 — each time because a number CI does not read cannot be kept true by anything except someone remembering |
...
| ⏳ Peers from the camera, class-agnostically | the boxes exist and are correctly placed; `PersonDetector` is configured `classes=("person",)` and drops them before anything downstream. **The range half is now wired** — `--static-detect-ground` ranges a box from where it touches the floor, so `--static-detect-height` and `--static-detect-width` are no longer asked for and an object nobody measured is rangeable between 0.81 m and a ceiling the stated pitch error sets. Still open: which of the false alarms the static map should hold rather than the tracker carry as a mover — 18% of frames on staged empty corridor, **56%** cross-day in a furnished one (76/136 after PR #91 repaired the derived split; it read 57% against 134) |
| ✅ An unrangeable frame stops the robot | issue [#72](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/72): below the range at which an object's floor contact leaves the frame — **0.806 m at the frame corner**, not the 0.719 m usually quoted — no estimator on this robot can say where it is, and the planner reads an empty obstacle list as an **open arena**. The navigator now holds with a stated reason when what ranging could not locate leaves less of the cone open than the robot could steer at. Fires on 4 of the 16 unrangeable ticks in the two committed live runs and costs the hero run 1 tick of 59. ⚠️ **Never fired in the arena** — `live05`, the run it is for, is not in this repository. [`evidence/2026-08-26-no-open-bearing/`](evidence/2026-08-26-no-open-bearing/) |
...
| ✅ Lite3 hardware commissioning | [#13](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/13): two-station camera validation (station-1 width +2.4%, two-station elevation fit −0.57°; the width-displacement FAIL is a detector min-area morphology artifact, recorded in the validated JSON), yaw primitives ±16000 in both directions (±0.857 rad/s, firmware-echoed), gait floor 0.30 m/s, actuator gain 1.07, loaded radius 0.40 m, operator-established standing/moving state (basic=6). Motor temperatures are absent from the high-level stream; live runs accept the bounded `--accept-no-motor-temperatures` waiver (≤120 s). |
| ⚠️ Lite3 live navigation, first walks | [#13](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/13): four `--live` runs on release v8 walked the robot — a 0.05→0.55 m/s ramp with the goal range closing 2.29→1.75 m (0.54 m walked) before a correct 58 s corridor hold on the box, and on run four **arrived**, 0.99 m from the chair. Battery gate enforced (50→47→35→28%), stall abort and safe park both exercised. **But the telemetry says these runs were the plain visual-nav goal follower, not MAPPO** — control reasons are only `goal`/`hold`, never `policy`/`veto-*` — and no run avoided the box: the holonomic policy's strafe intent was deleted by the `--max-vy 0` clamp, this Lite3's axis profile having no measured lateral primitive. [`evidence/2026-08-27-lite3-executable-avoidance/`](evidence/2026-08-27-lite3-executable-avoidance/) |
| ✅ Lite3 live MAPPO navigation, 2026-09-04 and 2026-09-07 | [#195](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/195): **MAPPO drove both Ventures**, from their own motion hosts. 2026-09-04: run `20260904T053300Z` reached its goal — 3.13 m in 8.5 s, **42 of 72 ticks on `policy`**, 0 veto-holds — and MAPPO held **71–100%** of decided ticks in every run where a robot moved ([handoff](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/195#issuecomment-5537171416)); robot 2 then arrived 0.50 m from its goal on attempt 1 after 11 s, 46/46 ticks ([comment](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/195#issuecomment-5537830539)). 2026-09-07: `--policy-mode raw` solved ~90% of the runs that had been failing, including one clean end-to-end, and became the default (#218); LITE3-A reached its goal with `--heading-servo goal`, while `off` walked 1.23x the direct line to a marker 3.29 m away (#212). ⚠️ **The 2026-09-04 arrival was to a dead-reckoned odom point, not a tracked marker**: as of the 15:30 CST handoff no run that day had sighted the 14 cm ArUco goal, which is unreadable beyond ~1.9 m. |
| ⚠️ Lite3 executable avoidance (turn-drive supervisor) | [#13](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/13): `mappo_drive.py --execution-supervisor turn-drive` — when a mapped static obstacle blocks the straight line to the goal, avoidance is re-expressed as pure turns and pure drives, the only motions this Lite3's measured axis profile can perform, and the planner's veto still judges the supervisor's command so a person stepping onto the detour holds the robot. Detour legs are tangent lines planned with a 2 cm execution margin over the static hard gap, and the second leg starts only when the straight line to the goal is clear, so the supervisor cannot deadlock against its own veto. Every planner tick now records the full decision chain (policy raw → envelope → gait floor → supervisor → final → axis preview) and the raw transport axes. **On hardware since 2026-09-04**: every live MAPPO run passed `--execution-supervisor turn-drive` ([#195](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/195#issuecomment-5537171416); the shipped venue profile [`dashboard/run-profile.lite3-venue.json`](dashboard/run-profile.lite3-venue.json)). ⚠️ **No record shows it executing a detour on a robot** — the September handoffs log arrivals, holds and stalls, never a supervisor leg. Since 2026-09-07 (#209), with both lateral primitives commissioned on robot 1, it stands down wherever a strafe can reach the target and still turns outside the forward hemisphere. |
