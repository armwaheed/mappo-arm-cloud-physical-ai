...
| ✅ Lite3 实机 MAPPO 导航，2026-09-04 与 2026-09-07 | [#195](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/195)：**MAPPO 驱动了两台 Venture**，运行在它们各自的运动主机上。2026-09-04：运行 `20260904T053300Z` 到达了目标 —— 8.5 秒走了 3.13 m，**72 个 tick 中有 42 个由 `policy` 控制**，0 次 veto-hold —— 并且在机器人移动过的每一次运行中，MAPPO 都占了已决策 tick 的 **71–100%**（[交接记录](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/195#issuecomment-5537171416)）；随后 2 号机在第 1 次尝试、11 秒后到达，距目标 0.50 m，46/46 个 tick（[评论](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/195#issuecomment-5537830539)）。2026-09-07：`--policy-mode raw` 解决了此前失败运行中的约 90%，其中包括一次完整的端到端运行，并成为默认值（#218）；LITE3-A 以 `--heading-servo goal` 到达了目标，而 `off` 在去往 3.29 m 外标记的路上走了直线距离的 1.23 倍（#212）。⚠️ **2026-09-04 的到达是到一个航位推算的 odom 点，而不是被跟踪的标记**：截至北京时间 15:30 的交接，当天没有任何一次运行识别到 14 cm 的 ArUco 目标，它在约 1.9 m 以外无法解码。 |
| ⚠️ Lite3 可执行的避障（转-走监督器） | [#13](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/13)：`mappo_drive.py --execution-supervisor turn-drive` —— 当一个已建图的静态障碍物挡住通往目标的直线时，避障被重新表达为纯转向与纯直行，也就是这台 Lite3 实测轴配置唯一能执行的动作；同时规划器的否决依然会审判监督器发出的指令，所以有人走进绕行路径时机器人仍会 hold。绕行段是按切线规划的，在静态硬间距之上留 2 cm 执行余量，而第二段只有在通往目标的直线畅通时才开始，因此监督器不会与自己的否决逻辑死锁。现在每一个规划器 tick 都记录完整的决策链（策略原始值 → 包络 → 步态下限 → 监督器 → 最终值 → 轴指令预览）以及原始传输轴值。**自 2026-09-04 起已在硬件上运行**：每一次实机 MAPPO 运行都带有 `--execution-supervisor turn-drive`（[#195](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/195#issuecomment-5537171416)；随仓库发布的场地配置 [`dashboard/run-profile.lite3-venue.json`](dashboard/run-profile.lite3-venue.json)）。⚠️ **没有任何记录显示它在机器人上执行过一次绕行** —— 九月的交接记录只记下了到达、hold 和失速，从未出现监督器的绕行段。自 2026-09-07（#209）起，1 号机的两个横向原语都已完成联调，只要横移能到达目标，它就让出控制；目标在前半球之外时它仍然转向。 |
