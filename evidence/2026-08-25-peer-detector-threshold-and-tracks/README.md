...
| "false-alarm rate is 18% on peer-free frames" | **Not real.** Every one of those alarms is a correct detection of an unlabelled peer. On genuinely peer-free frames of *this* capture the rate is **0/705 = 0.0%** at every threshold ≥ 0.14 — but see the ⛔ below: 705 staged negatives are not a room, and cross-day the same read fires on 57%. |
...
> ⛔ **"Free" is what this capture can say, and it is the half of the sentence that did
> not survive.** Recall is a property of the network and the 17 points stand. **The 0.0%
> is a property of the room.** All 705 negatives are one corridor, cleared and shot for
> the purpose, in the same session as the positives — a test sharing its conditions with
> the thing it tests. Scored **cross-day** on the Aug-20 held-out split in a furnished
> room, the same class-agnostic read at 0.25 fires on **57%** (76/134) of peer-free
> frames, and lowering the floor moves that the wrong way. See the 2026-08-26 section of
> [`detector/README.md`](../../detector/README.md), which supersedes any reading of this
> table as clearance to ship 0.15. **Nothing here changes the deployed threshold**, and
> the one-line prototxt edit below is a measurement harness, not a deployment step.
