...
| **The policy commands no yaw.** | It is a holonomic 2-D force. Without the heading servo the robot crabs and its 85° forward camera never looks anywhere new. The servo is on by default. ⚠️ **Pass `--no-heading-servo` anyway.** [#16](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/16) is an unstable feedback loop — the servo closes a heading loop through the frame the command is expressed in, so it chases its own rotated command — and it **put the robot into a wall three times**. `deploy/run-peer-supervised.sh` passes the flag for exactly this reason. Crabbing is the price of not hitting a wall; pay it until #16 is fixed. |
...
| the robot crabs sideways and never turns | `--no-heading-servo` was passed, or the servo's deadband is swallowing the error. ⚠️ If the flag was passed, this is **expected and correct** — see the yaw row above and [#16](https://github.com/armwaheed/mappo-arm-cloud-physical-ai/issues/16). Do not remove the flag to stop the crabbing. |
