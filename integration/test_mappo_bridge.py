...
def test_the_motion_growth_is_off_unless_a_caller_asks_for_it():
    """⛔ THE INVARIANT THAT KEEPS THIS PR INERT. The encoding below has never run on a
    robot, and the swerve it commands is below the lateral gait floor, so the default must
    leave the shipped behaviour bit-for-bit unchanged.

    Checked at BOTH levels, because only the second one is what production reaches:
    ``policy_objects`` called bare, and ``robot_input``, which every driver goes through.
    A default of 1.5 instead of 0.0 fails this on the first assert."""
    peer = {**PARKED_PEER, "vx": 0.0, "vy": 0.20}
    assert policy_objects(_tick(obstacles=[peer]))[0]["radius_m"] == PARKED_PEER["radius_m"]
    through_the_driver = robot_input(_tick(obstacles=[peer]))["stationary_objects"]
    assert through_the_driver[0]["radius_m"] == PARKED_PEER["radius_m"]
    # And the opt-in is the only thing that changes it, so the test above cannot pass
    # merely because the growth was deleted.
    opted_in = policy_objects(_tick(obstacles=[peer]),
                              motion_horizon_s=POLICY_MOTION_HORIZON_S)
    assert opted_in[0]["radius_m"] > PARKED_PEER["radius_m"]
...
def test_a_moving_peers_disc_is_grown_by_where_it_is_going():
    ...
    ⛔ Simulation only, and off by default — see the test above."""
    peer = {**PARKED_PEER, "vx": 0.0, "vy": 0.20}
    grown = policy_objects(_tick(obstacles=[peer]),
                           motion_horizon_s=POLICY_MOTION_HORIZON_S)[0]
...
def test_a_stationary_obstacle_is_not_grown_at_all():
    """No branch guards this: speed is zero, so the growth term vanishes even with the
    opt-in handed in. A mapped bin must not creep outward because a velocity field exists
    on its record."""
    horizon = POLICY_MOTION_HORIZON_S
    bin_ = policy_objects(_tick(obstacles=[BIN]), motion_horizon_s=horizon)[0]
    assert bin_["radius_m"] == BIN["radius_m"]
    parked = policy_objects(_tick(obstacles=[PARKED_PEER]), motion_horizon_s=horizon)[0]
...
def test_growth_is_isotropic_so_the_disc_stays_centred_on_the_peer():
    ...
    peer = {**PARKED_PEER, "vx": 0.0, "vy": 0.20}
    horizon = POLICY_MOTION_HORIZON_S
    still = policy_objects(_tick(obstacles=[PARKED_PEER]), motion_horizon_s=horizon)[0]
    moving = policy_objects(_tick(obstacles=[peer]), motion_horizon_s=horizon)[0]
...
def test_the_growth_is_bounded_by_the_speed_gate_that_precedes_it():
    ...
    grown = policy_objects(_tick(obstacles=[peer]), motion_horizon_s=horizon)[0]
    assert grown["radius_m"] < PARKED_PEER["radius_m"] + 0.376
    faster = {**PARKED_PEER, "vx": 0.0, "vy": POLICY_MAX_MOVER_SPEED_MPS + 0.05}
    assert policy_objects(_tick(obstacles=[faster]),
                          motion_horizon_s=horizon) == [], "should hold, not be grown"
